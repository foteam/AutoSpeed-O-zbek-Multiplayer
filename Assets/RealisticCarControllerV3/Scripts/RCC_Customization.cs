...
public class RCC_Customization : MonoBehaviour {
    ...
    public static void LoadStats(RCC_CarControllerV3 vehicle) {
        ...
        if (!CheckVehicle(vehicle))
            return;
        Debug.Log("Load Name: " + vehicle.transform.name.Split("(Clone)")[0]);
        Debug.Log("Original name: " + vehicle.transform.name);

        string vehicleName = vehicle.transform.name.Split("(Clone)")[0];
        Debug.Log("My: " + vehicleName);
        //  Loading all major settings of the vehicle with PlayerPrefs.
        SetFrontCambers(vehicle, PlayerPrefs.GetFloat(vehicleName + "_FrontCamber", vehicle.FrontLeftWheelCollider.camber));
        SetRearCambers(vehicle, PlayerPrefs.GetFloat(vehicleName + "_RearCamber", vehicle.RearLeftWheelCollider.camber));

        SetFrontSuspensionsDistances(vehicle, PlayerPrefs.GetFloat(vehicleName + "_FrontSuspensionsDistance", vehicle.FrontLeftWheelCollider.WheelCollider.suspensionDistance));
        SetRearSuspensionsDistances(vehicle, PlayerPrefs.GetFloat(vehicleName + "_RearSuspensionsDistance", vehicle.RearLeftWheelCollider.WheelCollider.suspensionDistance));

        SetFrontSuspensionsSpringForce(vehicle, PlayerPrefs.GetFloat(vehicleName + "_FrontSuspensionsSpring", vehicle.FrontLeftWheelCollider.WheelCollider.suspensionSpring.spring));
        SetRearSuspensionsSpringForce(vehicle, PlayerPrefs.GetFloat(vehicleName + "_RearSuspensionsSpring", vehicle.RearLeftWheelCollider.WheelCollider.suspensionSpring.spring));

        SetFrontSuspensionsSpringDamper(vehicle, PlayerPrefs.GetFloat(vehicleName + "_FrontSuspensionsDamper", vehicle.FrontLeftWheelCollider.WheelCollider.suspensionSpring.damper));
        SetRearSuspensionsSpringDamper(vehicle, PlayerPrefs.GetFloat(vehicleName + "_RearSuspensionsDamper", vehicle.RearLeftWheelCollider.WheelCollider.suspensionSpring.damper));

        SetMaximumSpeed(vehicle, PlayerPrefs.GetFloat(vehicleName + "_MaximumSpeed", vehicle.maxspeed));
        SetMaximumBrake(vehicle, PlayerPrefs.GetFloat(vehicleName + "_MaximumBrake", vehicle.brakeTorque));
        SetMaximumTorque(vehicle, PlayerPrefs.GetFloat(vehicleName + "_MaximumTorque", vehicle.maxEngineTorque));

        string drvtrn = PlayerPrefs.GetString(vehicleName + "_DrivetrainMode", vehicle.wheelTypeChoise.ToString());
        ...
        SetGearShiftingThreshold(vehicle, PlayerPrefs.GetFloat(vehicleName + "_GearShiftingThreshold", vehicle.gearShiftingThreshold));
        SetClutchThreshold(vehicle, PlayerPrefs.GetFloat(vehicleName + "_ClutchingThreshold", vehicle.clutchInertia));

        SetCounterSteering(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "_CounterSteering", vehicle.useCounterSteering));

        SetABS(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "_ABS", vehicle.ABS));
        SetESP(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "_ESP", vehicle.ESP));
        SetTCS(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "_TCS", vehicle.TCS));
        SetSH(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "_SH", vehicle.steeringHelper));

        SetNOS(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "NOS", vehicle.useNOS));
        SetTurbo(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "Turbo", vehicle.useTurbo));
        SetUseExhaustFlame(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "ExhaustFlame", vehicle.useExhaustFlame));
        SetRevLimiter(vehicle, RCC_PlayerPrefsX.GetBool(vehicleName + "RevLimiter", vehicle.useRevLimiter));

        if (PlayerPrefs.HasKey(vehicleName + "_WheelsSmokeColor"))
            SetSmokeColor(vehicle, 0, RCC_PlayerPrefsX.GetColor(vehicleName + "_WheelsSmokeColor"));

        if (PlayerPrefs.HasKey(vehicleName + "_HeadlightsColor"))
            SetHeadlightsColor(vehicle, RCC_PlayerPrefsX.GetColor(vehicleName + "_HeadlightsColor"));
